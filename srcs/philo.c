...
/*   Updated: 2026/03/15 18:55:13 by doleksiu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*philo_simulation(void *arg)
{
	...
	if (philo->philo_id % 2 == 0)
		get_time(data->time_to_eat / 2);
	while (1)
	{
		eating(data, data->philo_array, philo->philo_id);
		philo->eat_count++;
		if (data->num_of_philos == 1 || data->num_of_times_to_eat == philo->eat_count)
			break ;
...
int	check_if_dead(t_philo *philo_array, t_data *data, int *i)
{
	struct timeval current_time;
	int time;
